"""LambdaRank V2 blind-set inference driver: ABCDF+ALS → LightGBM rerank.

Extends F1 pipeline:
  1-5. Same A', B, C, D, F retrieval sources
  6.   ALS collaborative retrieval (128d, max-recent-5 weighted)
  7.   Weighted-RRF fuse (A=1, B=1, C=1, D=0.5, F=1, ALS=1, k=20) → top-200
  8.   27-feature LightGBM LambdaRank → top-20
...
def train_lambdarank(als_factors, als_track_ids, als_track_to_idx, track_pop):
    """Train LambdaRank on R12 payload (all 8000 cases) with V2 features."""
...
def build_row_features_lr(pool, user_messages, played, src_lists, als_vec,
                          ...
                          max_pop):
    """Build V2 27-feature vector for each candidate in pool."""
    n_feat = len(FEATURE_NAMES_V2)
...
def main(args: argparse.Namespace) -> None:
    ...
    log.info("=" * 70)
    log.info("LambdaRank V2 blind inference: ABCDF+ALS → LightGBM rerank")
    log.info("Sources: A'(qwen3) + B(bm25) + C(bm25) + D(qwen3) + F(cf-bpr) + ALS")
    log.info("Pool_k=%d  Features=%d  Ranker=LightGBM LambdaRank V2", POOL_K, len(FEATURE_NAMES_V2))
    log.info("=" * 70)
    ...
    log.info("Fusing ABCDF+ALS + LambdaRank for %d rows", len(pending))
    ...
    runtime_meta = {
        ...
        "driver": "run_inference_blind_lr.py",
        ...
        "ranker": "LightGBM LambdaRank V2 (300 rounds, 31 leaves, 27 features)",
        "local_cv5_estimate": "0.2015 (3-seed, expS2_lr_v2, pool_k=200)",
    }
