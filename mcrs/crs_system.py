...
class CRS_SYSTEM:
    ...
    def _retrieve(self, session_memory: list[dict], user_query: str) -> list[str]:
        ...
        if self.query_reformulator_aux:
            ...
            return self._rrf_merge(lists, topk=self.candidate_k)
        ...
        main_cands = self.retrieval.text_to_item_retrieval(query, topk=self.candidate_k)
        if self.use_track_sim_query:
            sim_cands = self._track_sim_candidates(session_memory)
            if sim_cands:
                return self._rrf_merge([main_cands, sim_cands], topk=self.candidate_k)
        return main_cands
