...
FEAT_DURATION = [
    "duration_diff_abs_seconds_to_last1",
    "duration_diff_abs_min_to_history",
    "duration_bucket_match_last1_30s",
    "duration_bucket_match_last3_30s",
    "duration_bucket_match_any_history_30s",
    "duration_ratio_to_last1",
]
FEAT_ALL_PLUS_DUR = FEAT_ALL_R39 + FEAT_DURATION  # 40 features
...
def build_feature_matrix(cases, payload, als_source, als_vecs, als_factors,
    ...
    for i, c in enumerate(cases):
        ...
        # Duration precomputation
        last1_dur = track_dur.get(played[-1], 0) if played else 0
        last3_durs = [track_dur.get(t, 0) for t in played[-3:]]
        last3_durs = [d for d in last3_durs if d > 0]
        ...
        for rank, tid in enumerate(pool[:POOL_K], start=1):
            ...
            # Duration features (indices 34-39)
            if include_duration:
                cand_dur = track_dur.get(tid, 0)
                dur_base = n_feat_base + 5  # = 34

                # [34] duration_diff_abs_seconds_to_last1
                if cand_dur > 0 and last1_dur > 0:
                    row[dur_base + 0] = abs(cand_dur - last1_dur) / 1000.0
                else:
                    row[dur_base + 0] = 0.0

                # [35] duration_diff_abs_min_to_history
                if cand_dur > 0 and all_hist_durs:
                    row[dur_base + 1] = min(abs(cand_dur - hd) for hd in all_hist_durs) / 1000.0
                else:
                    row[dur_base + 1] = 0.0

                # [36] duration_bucket_match_last1_30s
                if cand_dur > 0 and last1_dur > 0:
                    row[dur_base + 2] = 1.0 if abs(cand_dur - last1_dur) <= 30000 else 0.0
                else:
                    row[dur_base + 2] = 0.0

                # [37] duration_bucket_match_last3_30s
                if cand_dur > 0 and last3_durs:
                    row[dur_base + 3] = 1.0 if any(abs(cand_dur - hd) <= 30000 for hd in last3_durs) else 0.0
                else:
                    row[dur_base + 3] = 0.0

                # [38] duration_bucket_match_any_history_30s
                if cand_dur > 0 and all_hist_durs:
                    row[dur_base + 4] = 1.0 if any(abs(cand_dur - hd) <= 30000 for hd in all_hist_durs) else 0.0
                else:
                    row[dur_base + 4] = 0.0

                # [39] duration_ratio_to_last1
                if cand_dur > 0 and last1_dur > 0:
                    row[dur_base + 5] = np.clip(cand_dur / last1_dur, 0.2, 5.0)
                else:
                    row[dur_base + 5] = 1.0
...
def main():
    ...
    out_path = REPO / "exp" / "eval" / "expR49c_a_duration.json"
