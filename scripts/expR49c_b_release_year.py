...
def main():
    ...
    for i, c in enumerate(cases):
        ...
        # Year precomputation
        last1_year = track_year.get(played[-1], 0) if played else 0
        last1_decade = (last1_year // 10) * 10 if last1_year > 0 else 0

        # Last 3 history years
        last3_played = played[-3:] if len(played) >= 3 else played
        last3_years = [track_year.get(t, 0) for t in last3_played]
        last3_years_valid = [y for y in last3_years if y > 0]
        last3_decades = {(y // 10) * 10 for y in last3_years_valid}
        ...
        for rank, tid in enumerate(pool[:POOL_K], start=1):
            ...
            # Release year features (indices 34-39)
            cand_year = track_year.get(tid, 0)
            cand_decade = (cand_year // 10) * 10 if cand_year > 0 else 0

            # 1. year_diff_abs_to_last1
            if cand_year > 0 and last1_year > 0:
                row[YEAR_OFFSET + 0] = float(abs(cand_year - last1_year))
            else:
                row[YEAR_OFFSET + 0] = 100.0  # large sentinel for missing

            # 2. year_diff_abs_min_to_history
            if cand_year > 0 and all_hist_years_valid:
                row[YEAR_OFFSET + 1] = float(min(abs(cand_year - hy) for hy in all_hist_years_valid))
            else:
                row[YEAR_OFFSET + 1] = 100.0

            # 3. year_within_3_last1
            if cand_year > 0 and last1_year > 0:
                row[YEAR_OFFSET + 2] = 1.0 if abs(cand_year - last1_year) <= 3 else 0.0
            else:
                row[YEAR_OFFSET + 2] = 0.0

            # 4. year_within_3_last3
            if cand_year > 0 and last3_years_valid:
                row[YEAR_OFFSET + 3] = 1.0 if any(abs(cand_year - hy) <= 3 for hy in last3_years_valid) else 0.0
            else:
                row[YEAR_OFFSET + 3] = 0.0

            # 5. same_decade_last1
            if cand_decade > 0 and last1_decade > 0:
                row[YEAR_OFFSET + 4] = 1.0 if cand_decade == last1_decade else 0.0
            else:
                row[YEAR_OFFSET + 4] = 0.0

            # 6. same_decade_last3
            if cand_decade > 0 and last3_decades:
                row[YEAR_OFFSET + 5] = 1.0 if cand_decade in last3_decades else 0.0
            else:
                row[YEAR_OFFSET + 5] = 0.0
    ...
    out_path = REPO / "exp" / "eval" / "expR49c_b_release_year.json"
