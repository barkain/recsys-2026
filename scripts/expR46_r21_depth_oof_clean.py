...
"""R46 OOF-clean: R21 retrieval depth expansion with OOF-clean extended lists.

Replaces the contaminated R46 diagnostic. ALL depth configs now use OOF-sourced
R21 lists — no production model contamination.

For depth=300: uses original dev_r21_oof_lists.json (top-300, fold-clean)
For depth>300: uses cache/r46_r21_oof_extended.json sliced to depth (OOF-clean)

Reports: pool_hit, h7, cv5 for each depth config.
"""
from __future__ import annotations

import os
# Must be set before any C extension imports to avoid macOS fork crash
...
R46_OOF_EXT = REPO / "cache" / "r46_r21_oof_extended.json"
...
def main():
    ...
    t0 = time.time()
    print(f"{ts()} R46 OOF-CLEAN: Depth expansion evaluation")
    print("=" * 70)
    print("All depth configs use OOF-sourced R21 lists (NO production contamination)")
    ...
    print(f"{ts()} Loading R21 OOF lists (baseline top-300)...")
    with open(R21_OOF) as f:
        r21_oof = json.load(f)
    assert len(r21_oof) == n, f"OOF list count mismatch: {len(r21_oof)} vs {n}"

    # Load OOF-clean extended R21 lists (top-2000)
    print(f"{ts()} Loading OOF-clean extended R21 lists (top-2000)...")
    with open(R46_OOF_EXT) as f:
        r21_oof_extended = json.load(f)
    assert len(r21_oof_extended) == n, f"Extended list count mismatch: {len(r21_oof_extended)} vs {n}"

    # Popularity
    print(f"{ts()} Building popularity stats...")
    track_pop = build_popularity_stats()
    max_pop = max(track_pop.values()) if track_pop else 1

    # ALS (must be built BEFORE importing datasets to avoid loky segfault)
    ...
    for c in cases:
        ...
        if sv is not None:
            sc = als_factors @ sv
            for t_id in played:
                if t_id in als_track_to_idx:
                    sc[als_track_to_idx[t_id]] = -np.inf
    ...
    # Load album mapping (AFTER ALS to avoid loky/datasets segfault)
    ...
    print(f"  {n_feat} features ({n_feat_base} base + {len(FEAT_ALBUM)} album)")
    ...
    for r21_depth in DEPTH_CONFIGS:
        ...
        # ALL depths use OOF-sourced lists
        if r21_depth == 300:
            r21_source = r21_oof
            print("  Using R21 OOF lists (fold-clean, top-300)")
        else:
            r21_source = [lst[:r21_depth] for lst in r21_oof_extended]
            print(f"  Using OOF-clean extended R21 lists (top-{r21_depth})")
        ...
        for i, c in enumerate(cases):
            ...
            prior = [(1.0 / (j + 1), ta.get(t_id, ""), tt.get(t_id, set()))
                     for j, t_id in enumerate(reversed(played))]
            sv = als_vecs[i]
            pool_artists = [ta.get(tid, "") for tid in pool[:POOL_K]]
            artist_counts = Counter(a for a in pool_artists if a)
            # OOF-clean r21_rank_map: use OOF source for ALL depths
            ...
            last3_albums = {track_album.get(t_id, "") for t_id in played[-3:]} - {""}
            all_albums = [track_album.get(t_id, "") for t_id in played]
        ...
        h7_same = [i for i in h7 if ta.get(cases[i]["gt"], "") and
                   ...
                   {ta.get(t_id, "") for t_id in cases[i]["music_turns"]}]
    ...
    sep = "=" * 70
    print("\n" + sep)
    print("R46 OOF-CLEAN R21 DEPTH EXPANSION RESULTS")
    print(sep)

    base = results["R21_top300"]
    header = f"{'Config':<15s} {'pool_hit':>9s} {'h7':>9s} {'cv5':>9s} "
    header += f"{'same':>9s} {'diff':>9s} {'dh7':>9s}"
    print("\n" + header)
    print("-" * 70)
    for name, r in results.items():
        dh7 = r["h7"] - base["h7"]
        print(f"  {name:<13s} {r['pool_hit']:9.4f} {r['h7']:9.5f} {r['cv5']:9.5f} "
              f"{r['same']:9.5f} {r['diff']:9.5f} {dh7:+9.5f}")

    # Gate check
    print("\n--- GATE CHECK ---")
    print("  Expected baseline: h7=0.24298, pool_hit=0.6000")
    print(f"  Actual baseline:   h7={base['h7']:.5f}, pool_hit={base['pool_hit']:.4f}")
    baseline_ok = abs(base["h7"] - 0.24298) < 0.005 and abs(base["pool_hit"] - 0.6000) < 0.01
    print(f"  Baseline reproduction: {'OK' if baseline_ok else 'MISMATCH - INVESTIGATE'}")

    best_name = max(results, key=lambda k: results[k]["h7"])
    best = results[best_name]
    dh7_best = best["h7"] - base["h7"]
    print(f"\n  Best config: {best_name}")
    print(f"  Best h7: {best['h7']:.5f}")
    print(f"  Delta h7 vs baseline: {dh7_best:+.5f}")
    gate_pass = dh7_best >= 0.010
    print(f"  GATE (delta h7 >= +0.010): {'PASS' if gate_pass else 'FAIL'}")
    ...
    for name, r in results.items():
        ...
        dh7 = r["h7"] - base["h7"]
        dcv5 = r["cv5"] - base["cv5"]
        print(f"  {name}: delta_pool_hit={dpool:+.4f}  delta_h7={dh7:+.5f}  delta_cv5={dcv5:+.5f}")

    # Save results
    output = {
        "experiment": "R46_oof_clean",
        "description": "R21 depth expansion with OOF-clean lists (no production contamination)",
        "model_manifest": {
            "fold_0": "cache/r21_supervised/fold_0",
            "fold_1": "cache/r21_supervised/fold_1",
            "fold_2": "cache/r21_production/oof/model_fold_2",
            "fold_3": "cache/r21_production/oof/model_fold_3",
            "fold_4": "cache/r21_production/oof/model_fold_4",
        },
        "fold_split": "grouped_session_folds(sessions, seed=0)",
        ...
        "gate_pass": gate_pass,
        "baseline_reproduction_ok": baseline_ok,
        "created_at": datetime.now().isoformat(),
    }

    out_path = REPO / "exp" / "eval" / "expR46_r21_depth_oof_clean.json"
