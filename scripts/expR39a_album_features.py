...
"""R39a: Album-aware LambdaRank.

Add album match features based on R38 forensics showing 20% of miss cases
are same-album GTs.
"""
...
def load_track_albums():
    """Load track_id -> album_name mapping."""
...
def main():
    t0 = time.time()
    print(f"{ts()} R39a: Album-Aware LambdaRank")
    print("=" * 70)
    ...
    configs = [
        ...
        ("+album", FEAT_BASE + FEAT_ALBUM, True),
    ]

    results = {}
    for config_name, feat_names, use_album in configs:
        ...
        for i, c in enumerate(cases):
            ...
            last1_album = ""
            last3_albums: set[str] = set()
            album_hist_counts: Counter[str] = Counter()
            if use_album:
        ...
        # Feature importance for album config (last fold model)
        if use_album:
    ...
    sep = "=" * 70
    print(f"\n{sep}")
    print("R39a ALBUM FEATURES")
    print(sep)
    base = results["baseline"]
    album = results["+album"]
    dh7 = album["h7"] - base["h7"]
    dsame = album["same"] - base["same"]
    print(f"  baseline: h7={base['h7']:.5f}  same={base['same']:.5f}  diff={base['diff']:.5f}")
    print(f"  +album:   h7={album['h7']:.5f}  same={album['same']:.5f}  diff={album['diff']:.5f}")
    print(f"  Δh7={dh7:+.5f}  Δsame={dsame:+.5f}  Δdiff={album['diff']-base['diff']:+.5f}")
    g = dh7 >= 0.005
    print(f"\n  GATE: Δh7 >= +0.005: {dh7:+.5f} {'PASS' if g else 'FAIL'}")

    out_path = REPO / "exp" / "eval" / "expR39a_album_features.json"
