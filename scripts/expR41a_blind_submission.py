...
"""R41a: Rare-tag LambdaRank blind submission.

R39 album baseline + 7 rare-tag features = 41 total features.

Steps:
1. Train production LambdaRank with album + rare-tag features on full dev
...
FEAT_ALL = FEAT_BASE + FEAT_ALBUM + FEAT_RARE_TAG
...
def build_features_with_album_raretag(cases, payload, als_source, als_vecs, als_factors,
                                       als_track_to_idx, track_pop, r21_source,
                                       track_album, catalog_tags, tag_idf, rare_tags):
    """Build feature matrix with album + rare-tag features for all dev cases."""
    n = len(cases)
    ...
    n_rare_offset = n_base + len(FEAT_ALBUM)
...
def main():
    print(f"{ts()} R41a: Rare-Tag Blind Submission")
    print("=" * 70)

    # Step 1: Train production LambdaRank with album + rare-tag features
    ...
    model_path = REPO / "cache" / "r41a_lr_model.txt"
    ...
    # Save tag data for blind phase
    tag_cache = REPO / "cache" / "r41a_tag_data.pkl"
    with open(tag_cache, "wb") as f:
        pickle.dump({
            "catalog_tags": catalog_tags,
            "tag_idf": tag_idf,
            "rare_tags": rare_tags,
        }, f)
    print(f"  Tag data saved to {tag_cache}")
...
def phase_blind():
    ...
    with open(REPO / "cache" / "r41a_track_pop.json") as f:
        track_pop = json.load(f)

    als_data = np.load(REPO / "cache" / "r41a_als.npz", allow_pickle=True)
    ...
    with open(REPO / "cache" / "r41a_payload_maps.pkl", "rb") as f:
        maps = pickle.load(f)
    ...
    # Load tag data
    with open(REPO / "cache" / "r41a_tag_data.pkl", "rb") as f:
        tag_data = pickle.load(f)
    catalog_tags = tag_data["catalog_tags"]
    tag_idf = tag_data["tag_idf"]
    rare_tags = tag_data["rare_tags"]
    print(f"  Tags: {len(catalog_tags)} tracks, {len(rare_tags)} rare tags")
    ...
    blind_cache = REPO / "cache" / "r41a_blind_features.pkl"
...
def phase_score():
    ...
    lr_model = lgb.Booster(model_file=str(REPO / "cache" / "r41a_lr_model.txt"))
    track_album = load_track_albums()

    with open(REPO / "cache" / "r41a_blind_features.pkl", "rb") as f:
    ...
    if r39_path.exists():
        ...
        top1_changed = sum(1 for r in results
                          if r["predicted_track_ids"][0] != r39.get(r["session_id"], {}).get("predicted_track_ids", [""])[0])
        overlaps = [len(set(r["predicted_track_ids"]) &
                       set(r39.get(r["session_id"], {}).get("predicted_track_ids", [])))
                   for r in results]
        print(f"  vs R39: top-1 changed={top1_changed}/80, avg overlap={np.mean(overlaps):.1f}/20")
    ...
    out_json = BLIND_OUT / "r41a_raretag_submission.json"
    with open(out_json, "w") as f:
        json.dump(results, f, indent=2)

    out_zip = BLIND_OUT / "r41a_raretag_submission.zip"
