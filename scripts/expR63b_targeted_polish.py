...
"""R63b: targeted response-side polish on top of R63.

Only the weakest R63 response rows are regenerated. Track IDs remain copied
from R63/R54c and are validated bitwise against R54c before packaging.

Outputs:
  exp/inference/blind_a/r63b_rows_persisted.jsonl
  exp/inference/blind_a/r63b_rows_final.jsonl
  exp/inference/blind_a/r63b_targeted_polish_submission.zip
  exp/inference/blind_a/r63b_targeted_polish_submission.metadata.json
  docs/r63b_targeted_polish_result.md
"""
...
PERSISTED_ROWS = OUT_DIR / "r63b_rows_persisted.jsonl"
FINAL_ROWS = OUT_DIR / "r63b_rows_final.jsonl"
OUT_ZIP = OUT_DIR / "r63b_targeted_polish_submission.zip"
OUT_METADATA = OUT_DIR / "r63b_targeted_polish_submission.metadata.json"
OUT_DOC = REPO / "docs" / "r63b_targeted_polish_result.md"

MODEL_ID = "claude-opus-4-7"
PROMPT_VERSION = "R63b targeted polish v1; selected weakest R63 rows only"
TARGET_REGEN_COUNT = 25
MIN_REGEN_ROWS = 15
MAX_REGEN_ROWS = 25
...
def render_prompt(
    ...
    pieces = [
        ...
        "",
        "Previous R63 response being polished. Keep any true specifics, but make "
...
def load_r63_metadata() -> dict[str, Any] | None:
    if not R63_METADATA.exists():
        return None
    try:
        return json.loads(R63_METADATA.read_text(encoding="utf-8"))
    except Exception:
        return None
...
def selected_samples(
    ...
    for entry in selected[:limit]:
        ...
        samples.append({
            ...
            "r54c_response": r54c_rows[idx]["predicted_response"],
            "r63_response": r63_rows[idx]["predicted_response"],
            "r63b_response": final_rows[idx]["predicted_response"],
        })
    return samples
...
def write_metadata(
    ...
    label = (
        "R63b targeted response polish | base=R63 | tracks=R54c | "
        f"{len(accepted_indices)} rows regenerated | LexDiv={lexdiv:.4f} | "
        "purpose=push LLM judge 4.80 -> 4.85+"
    )
    ...
    metadata = {
        ...
        "base": {
            "rows_file": str(R63_ROWS.relative_to(REPO)),
            "submission_zip": "exp/inference/blind_a/r63_response_only_submission.zip",
            "metadata_file": str(R63_METADATA.relative_to(REPO)),
            "blind_result": {
                "composite": 0.6180,
                "llm_judge": 4.80,
                "lexdiv": 0.8379,
        ...
        "source_tracks": {
            ...
            "constraint": "R63b predicted_track_ids copied from R63/R54c, unchanged.",
        },
        ...
        "lexdiv_policy": {
            ...
            "blind_scorer_note": (
                "R63 local LexDiv was 0.8256 while blind LexDiv was 0.8379; "
        ...
        "selection": {
            "policy": (
                ...
                "Selected only the top ranked weak rows; all other R63 responses kept."
            ),
        ...
        "generation": {
            ...
            "non_selected_rows_kept_from_r63": 80 - len(selected),
        ...
        "before_after_samples": selected_samples(
            selected, r54c_rows, r63_rows, final_rows, catalog
        ),
    }
...
def write_result_doc(
    ...
    doc = f"""# R63b Targeted Polish Result
...
- Non-selected rows kept from R63: {metadata['generation']['non_selected_rows_kept_from_r63']}
...
- Opus API calls for R63b run: {metadata['usage']['calls']}
- Estimated R63b run cost: ${metadata['usage']['estimated_cost_usd']:.4f}
...
def main() -> None:
    ...
    if args.model != MODEL_ID:
        raise SystemExit(
            f"Refusing model override {args.model!r}; R63b requires {MODEL_ID!r}."
        )
    ...
    print(f"{ts()} R63b targeted response polish")
    print(f"{ts()} Loading R63 rows, R54c tracks, Blind-A sessions, and catalog")
    r63_rows = read_jsonl(R63_ROWS)
    if len(r63_rows) != 80:
        raise RuntimeError(f"Expected 80 R63 rows, found {len(r63_rows)}")
    r54c_rows = r63.load_r54c_rows()
    blind_by_sid = r63.load_blind_by_sid()
    catalog = r63.load_catalog()
    r63_metadata = load_r63_metadata()

    print(f"{ts()} Auditing R63 responses for targeted weak-row selection")
    audit = weakness_audit(r63_rows, blind_by_sid, catalog)
    ...
    final_rows = copy.deepcopy(r63_rows)
    ...
    for n_done, idx in enumerate(sorted(selected_indices), start=1):
        ...
        if accepted:
            ...
            failures_after_retries.append({
                ...
                "kept_original_r63_response": True,
            })
        attempts.append({
            "row_index": idx,
            "session_id": sid,
