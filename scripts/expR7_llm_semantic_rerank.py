#!/usr/bin/env python3
# ruff: noqa: E402,T201
"""R7: Cheap LLM semantic rerank over ABCDFG candidate pool.

Tests whether Haiku can select better top-20 from the heterogeneous pool
where deterministic features fail.
...
of candidate tracks, select and rank the TOP {topk} most relevant tracks.

Your #1 ranked track must be the single BEST match for the user's immediate request.

Scoring criteria (strict priority):
1. Explicit user request — if the user named a specific artist/track, that MUST be rank 1
2. Genre/style/mood match — fits what the user described
3. Artist similarity — similar to artists mentioned or played
4. Era/decade — matches any time period mentioned
5. Thematic fit — lyrical themes, energy, tempo cues

IMPORTANT: Tracks already played in the conversation (shown as "Music:" turns) \
should NOT be ranked highly unless the user explicitly asks again.

Rules:
- Return exactly ONE JSON array of {topk} candidate numbers
- Each number is an integer between 1 and {n_candidates}
- No UUIDs, no explanations, no prose — just the JSON array"""
...
Candidate tracks ({n_candidates} total):
{candidates_text}

Return exactly one JSON array of {topk} integers (candidate numbers 1..{n_candidates})."""
...
def main():
    ...
    out_path = REPO_ROOT / "exp" / "eval" / "expR7_llm_semantic_rerank.json"
