...
"""R41c: BM25 bucket preservation features on R39 baseline.

R40 forensics found bm25_top20_demoted in 40/176 miss cases (22.7%).
Add bucket flags so LR can learn to preserve high-BM25 candidates.
Note: we already have B_rank_inv and C_rank_inv as smooth features.
The forensic signal suggests bucket/flag features may add value.
"""
...
def main():
    t0 = time.time()
    print(f"{ts()} R41c: BM25 Bucket Preservation Features on R39 Baseline")
    print("=" * 70)
    ...
    n_bm25_offset = n_feat_base + len(FEAT_ALBUM)

    configs = [
        ("R39_baseline", FEAT_BASE + FEAT_ALBUM, False),
        ("R39+bm25_bucket", FEAT_BASE + FEAT_ALBUM + FEAT_BM25_BUCKET, True),
    ]

    results = {}
    config_ndcg = {}

    for config_name, feat_names, use_bm25 in configs:
        ...
        for i, c in enumerate(cases):
            ...
            for rank, tid in enumerate(pool[:POOL_K], start=1):
                ...
                # BM25 bucket features
                if use_bm25:
                    b_rank = src_rank["B"].get(tid)
                    c_rank = src_rank["C"].get(tid)
                    best_bm25_rank = min(
                        b_rank if b_rank is not None else 9999,
                        c_rank if c_rank is not None else 9999,
                    )
                    row[n_bm25_offset + 0] = 1.0 / best_bm25_rank if best_bm25_rank < 9999 else 0.0
                    row[n_bm25_offset + 1] = 1.0 if best_bm25_rank <= 20 else 0.0
                    row[n_bm25_offset + 2] = 1.0 if best_bm25_rank <= 50 else 0.0
                    if best_bm25_rank <= 10:
                        row[n_bm25_offset + 3] = 4.0
                    elif best_bm25_rank <= 20:
                        row[n_bm25_offset + 3] = 3.0
                    elif best_bm25_rank <= 50:
                        row[n_bm25_offset + 3] = 2.0
                    elif best_bm25_rank <= 100:
                        row[n_bm25_offset + 3] = 1.0
                    else:
                        row[n_bm25_offset + 3] = 0.0
        ...
        if use_bm25 and mdl is not None:
    ...
    out_path = REPO / "exp" / "eval" / "expR41c_bm25_bucket.json"
