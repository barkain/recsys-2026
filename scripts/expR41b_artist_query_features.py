...
"""R41b: Artist-in-query features on R39 baseline.

R40 forensics found artist_in_query in 36/176 miss cases (20.5%).
Add features detecting when the candidate artist is mentioned in the query.
"""
...
def load_track_artist_names() -> dict[str, str]:
    """Load track_id -> full artist name string for phrase matching."""
    from datasets import DownloadConfig, load_dataset
    ds = load_dataset("talkpl-ai/TalkPlayData-Challenge-Track-Metadata",
                      download_config=DownloadConfig(local_files_only=True))["all_tracks"]
    result: dict[str, str] = {}
    ...
    print(f"  Artist names: {len(result)} tracks, {non_empty} with names")
    return result


def artist_token_set(name: str) -> set[str]:
    """Tokenize artist name for overlap matching."""
    return {w.lower() for w in re.findall(r"[a-zA-Z0-9]+", name) if len(w) > 1}


def main():
    t0 = time.time()
    print(f"{ts()} R41b: Artist-in-Query Features on R39 Baseline")
    print("=" * 70)
    ...
    n_aq_offset = n_feat_base + len(FEAT_ALBUM)

    configs = [
        ("R39_baseline", FEAT_BASE + FEAT_ALBUM, False),
        ("R39+artist_query", FEAT_BASE + FEAT_ALBUM + FEAT_ARTIST_QUERY, True),
    ]

    results = {}
    config_ndcg = {}

    for config_name, feat_names, use_aq in configs:
        ...
        for i, c in enumerate(cases):
            ...
            for rank, tid in enumerate(pool[:POOL_K], start=1):
                ...
                # Artist-in-query features
                if use_aq:
                    artist_name = track_artist_name.get(tid, "")
                    if artist_name and len(artist_name) > 2:
                        aname_lower = artist_name.lower()
                        row[n_aq_offset + 0] = 1.0 if aname_lower in current_query_lower else 0.0
                        row[n_aq_offset + 1] = 1.0 if aname_lower in full_conv_lower else 0.0
                        a_toks = artist_token_set(artist_name)
                        if a_toks and current_query_toks:
                            row[n_aq_offset + 2] = len(a_toks & current_query_toks) / len(a_toks)
                        escaped = re.escape(aname_lower)
                        row[n_aq_offset + 3] = 1.0 if re.search(r'\b' + escaped + r'\b', current_query_lower) else 0.0
    ...
    out_path = REPO / "exp" / "eval" / "expR41b_artist_query.json"
