...
# R68.1 v2: real train cosines + train OOF lists
R68_TRAIN_LISTS = R68_DIR / "oof_r68_lists_train.json"
R68_QUERY_TRAIN_EMBS = R68_DIR / "query_embeddings_train.npy"
R68_QUERY_TRAIN_CASE_IDS = R68_DIR / "query_embeddings_train_case_ids.json"
W0_STATS = REPO / "exp" / "eval" / "expR68_r54_reference_stats.pkl"
W0_AGG = REPO / "exp" / "eval" / "expR68_r54_aggregate.json"

OUT_JSON = REPO / "exp" / "eval" / "expR68_phase0_fold0_eval_v2.json"
OUT_MD = REPO / "docs" / "r68_phase0_fold0_result_v2.md"
...
def main() -> None:
    ...
    # R68.1 v2 — Load TRAIN-side R68 artifacts (computed post-Phase0 from saved model)
    print(f"{ts()} Loading R68 TRAIN artifacts (v2: real train cosines + train OOF lists)...", flush=True)
    if not (R68_TRAIN_LISTS.exists() and R68_QUERY_TRAIN_EMBS.exists() and R68_QUERY_TRAIN_CASE_IDS.exists()):
        raise RuntimeError("v2 requires train artifacts: oof_r68_lists_train.json, "
                           "query_embeddings_train.npy, query_embeddings_train_case_ids.json")
    r68_train_lists_raw = json.load(open(R68_TRAIN_LISTS))  # {case_id_str: [[tid, score], ...]}
    r68_query_train_embs = np.load(R68_QUERY_TRAIN_EMBS)
    r68_query_train_case_ids = json.load(open(R68_QUERY_TRAIN_CASE_IDS))  # list aligned to embs rows
    print(f"  train_lists: {len(r68_train_lists_raw)} cases  "
          f"train_query_embs: {r68_query_train_embs.shape}", flush=True)

    # Build case_idx -> train_qemb_row + case_idx -> train_r68_list
    def _case_id_str(c: dict) -> str:
        return c.get("case_id") or f"{c['session_id']}_{c['turn_number']}"
    cid_to_qemb_row = {cid: ki for ki, cid in enumerate(r68_query_train_case_ids)}
    case_to_qemb_train_row: dict[int, int] = {}
    r68_train_lists_by_case: dict[int, list[tuple[str, float]]] = {}
    for i, c in enumerate(cases):
        cid = _case_id_str(c)
        if cid in cid_to_qemb_row:
            case_to_qemb_train_row[i] = cid_to_qemb_row[cid]
            r68_train_lists_by_case[i] = [
                (str(t), float(s)) for t, s in r68_train_lists_raw[cid]]
    print(f"  mapped {len(case_to_qemb_train_row)} train cases to query-emb rows  "
          f"({len(r68_train_lists_by_case)} train R68 lists)", flush=True)
    ...
    # R68.1 v2: real train query embeddings + train OOF lists now available.
    print(f"  v2: using REAL train r68_cosine + train R68 OOF lists (bias removed)",
          flush=True)
    ...
    for ki, i in enumerate(train_idx):
        ...
        src_lists_r68 = dict(src_lists_base)
        src_lists_r68.pop("R54", None)
        # R68.1 v2: REAL train R68 source list + REAL r68_cosine score map.
        train_r68_list = r68_train_lists_by_case.get(i, [])
        src_lists_r68["R68"] = [t for t, _ in train_r68_list[:POOL_K]]
        train_r68_scores = {t: float(s) for t, s in train_r68_list}
        pool = weighted_rrf(src_lists_r68, SW_R68, topk=POOL_K, k=RRF_K)
        gt = cases[i]["gt"]
        gi = pool.index(gt) if gt in pool else -1
        if gi >= 0:
            gt_in_pool_train += 1
        feats = featurize_case(i, src_lists_r68, pool, train_r68_scores)
    ...
    report = {
        ...
        "sibling_lr": {
            ...
            "notes": (
                "R68.1 v2 corrected sibling LR: trained with REAL train r68_cosine "
                "(from saved fold-0 R68 model encoding the 6400 fold-0 TRAIN queries) "
                "and REAL train R68 source list (cosine top-300 over track embeddings). "
                "This removes the train-zero-cosine bias from the original Phase 0."
            ),
        },
        "version": "v2 (R68.1 corrected)",
        "v2_changes": "Real train r68_cosine + real train R68 OOF lists. "
                       "Train query embeddings encoded post-Phase 0 from the saved fold-0 model.",
        "notes": (
            "R68.1 v2: substitutes 3 LR feature columns (r54_* -> r68_*); pool "
            "admission unchanged. This is NOT matched-pool retraining. The train-time "
            "zero-cosine bias from the original Phase 0 has been removed by encoding "
            "fold-0 TRAIN queries with the saved fold-0 model and computing real top-300 "
            "via cosine search."
        ),
    }
